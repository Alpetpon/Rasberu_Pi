...
#вспомогательная функция для выборки user_id из таблицы all_users по username для дальнейшей регистрации пользователя админом
...
async def add_user_by_admin_to_user_table(user_id, username, comment, role, current_time, permis):
    async with aiosqlite.connect('userdata.db') as db:
        await db.execute('INSERT INTO users (user_id, username, comment, role, last_action, permissions) VALUES (?,?,?,?,?,?)', (user_id, username, comment, role, current_time, permis))
        await db.commit()
